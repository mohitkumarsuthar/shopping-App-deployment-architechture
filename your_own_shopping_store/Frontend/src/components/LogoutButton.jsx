...
const LogoutButton = ({ onClick, className = '' }) => {
  ...
  const [styles, setStyles] = useState(logoutButtonStates['default']);
  const isAnimating = useRef(false);

  const updateState = (newState) => {
    setState(newState);
    if (logoutButtonStates[newState]) {
        setStyles(prev => ({ ...prev, ...logoutButtonStates[newState] }));
    }
  };

  const handleMouseEnter = () => {
    if (state === 'default' && !isAnimating.current) {
      updateState('hover');
    }
  };

  const handleMouseLeave = () => {
    if (state === 'hover' && !isAnimating.current) {
      updateState('default');
    }
  };
  ...
  const handleClick = (e) => {
    ...
    setTimeout(() => {
        setClasses(prev => prev + ' door-slammed');
        updateState('walking2');

        setTimeout(() => {
            setClasses(prev => prev + ' falling');
        ...
        }, parseInt(logoutButtonStates['walking2']['--figure-duration'])); // 400

    }, parseInt(logoutButtonStates['walking1']['--figure-duration'])); // 300
  };

  return (
    <button
      className={`logoutButton logoutButton--dark ${classes} ${className}`}
      style={styles}
